/*
gcc phone2.c -o phone2 $(pkg-config --cflags --libs gtk+-3.0)
*/
...
int socket_OK = 0;
time_t seconds = 0;
pthread_t recv_play_tid, rec_send_tid, client_call_tid, server_tid, play_incoming_call_tid;
...
void *play_incoming_call() {
	...
	int n = 44100;
	int16_t samples[n];
	unsigned buf=sizeof(samples);
	for (int i = 0; i < n; i++) {
		samples[i] = (int16_t) A*sin(2*M_PI*f*(double)i/(double)44100);
	}
	while(1) {
		int done = fwrite(samples, buf, 1, fp_play);
		if (n < 0) { perror("fwrite"); }
		sleep(2);
	}
	pclose(fp_play);
	return 0;
...
gboolean cb_answer_call() {
	printf("Answer Call\n");
	pthread_cancel(play_incoming_call_tid);
	gtk_widget_destroy(dialog);
	...
	label = gtk_label_new("Call in progress!");
...
gboolean incoming_call_dialog() {
	...
	// Create the widgets
	flags = GTK_DIALOG_DESTROY_WITH_PARENT | GTK_DIALOG_MODAL;
	dialog = gtk_dialog_new_with_buttons("Message", GTK_WINDOW(window), flags, "ANSWER CALL", 1, NULL);
	content_area = gtk_dialog_get_content_area(GTK_DIALOG(dialog));
...
void *server_start() {
	...
	while (1) {
		...
		pthread_create(&play_incoming_call_tid, NULL, play_incoming_call, NULL);
		// pthread_create(&rec_send_tid, NULL, rec_send, NULL);
	}
}
